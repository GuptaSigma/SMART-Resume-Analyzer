...
class AIDetector:
    ...
    def _train_synthetic_model(self) -> None:
        ...
        human = np.column_stack([
            rng.normal(75, 20, n),
            rng.poisson(0.1, n),
            rng.normal(0.40, 0.25, n),
            rng.normal(0.75, 0.30, n),
            rng.normal(250, 100, n),
            rng.poisson(6.0, n),
            rng.normal(4.6, 0.7, n),
            rng.beta(2.0, 5.0, n),
            rng.normal(0.38, 0.14, n),
            rng.normal(0.04, 0.06, n),
            rng.poisson(0.5, n),
            rng.normal(0.15, 0.08, n),
            rng.normal(0.78, 0.12, n),
            rng.normal(4.5, 0.8, n),
            rng.normal(0.05, 0.04, n),
            rng.normal(0.90, 0.08, n),
            rng.normal(0.72, 0.18, n),
            rng.normal(5.0, 2.2, n),
            rng.normal(7.0, 1.5, n),
            rng.normal(250, 100, n),
            rng.normal(0.3, 0.3, n),
            rng.normal(5.5, 2.5, n),
            rng.normal(6.5, 3.0, n),
        ])
        ai = np.column_stack([
            rng.normal(35, 6, n),
            rng.poisson(6.0, n),
            rng.normal(0.92, 0.04, n),
            rng.normal(0.12, 0.06, n),
            rng.normal(60, 18, n),
            rng.poisson(0.1, n),
            rng.normal(3.0, 0.25, n),
            rng.beta(4.0, 2.0, n),
            rng.normal(0.60, 0.05, n),
            rng.normal(0.45, 0.15, n),
            rng.poisson(9.0, n),
            rng.normal(0.02, 0.02, n),
            rng.normal(0.50, 0.07, n),
            rng.normal(6.2, 0.5, n),
            rng.normal(0.25, 0.08, n),
            rng.normal(0.55, 0.10, n),
            rng.normal(0.35, 0.12, n),
            rng.normal(0.3, 0.3, n),
            rng.normal(4.5, 0.6, n),
            rng.normal(55, 15, n),
            rng.normal(4.5, 1.5, n),
            rng.normal(0.2, 0.2, n),
            rng.normal(0.8, 0.6, n),
        ])
        X = np.vstack([human, ai]).astype(float)
        y = np.array([0] * n + [1] * n)
        # (Clipping for feature scaling as in your original code...)
    ...
    def analyze_resume(self, text: str, candidate_name: str, resume_filename: str) -> Dict[str, Any]:
        ...
        # FINAL: AI Confidence is model certainty (not the same as AI percentage)
