...
class ResumeAnalyzer:
    ...
    def calculate_match_score(self, candidate_skills: List[SkillHit], target_skills: List[str],
        ...
        # Bonus for number of matched skills
    ...
    def _extract_skills_from_jd(self, jd_content: str) -> List[str]:
        """Extract skills/keywords from a given Job Description text."""
        ...
        # Additional extraction: look for common skill patterns in JD
        ...
        return list(set(jd_skills))  # Remove duplicates

    # ====== ENHANCED HR REQUIREMENTS CHECK METHOD ======
    def _check_hr_requirements(self, hr_requirements: str, candidate) -> List[str]:
        """Check custom HR requirements like mandatory skills or min experience."""
        issues = []
        ...
        for req in reqs:
            ...
            # Check for minimum experience requirements
            if "min" in req_lower and ("year" in req_lower or "yr" in req_lower or "experience" in req_lower):
                # Extract number (e.g., "min 3 years", "minimum 5 years experience")
            ...
            # Check for maximum experience (optional)
            ...
            # Check for mandatory skills (default case)
            else:
                # Clean the requirement (remove "mandatory", "must have", etc.)
                clean_req = re.sub(r'\b(mandatory|must have|required|should have)\b', '', req_lower).strip()
                
                if clean_req:
                    # Check if skill exists in candidate's skills
    ...
    def analyze_resume(self, file, target_role: str = "", jd_content: str = "", hr_requirements: str = "") -> Dict[str, Any]:
        try:
            ...
            if target_role and target_role.lower() == "other":
                ...
                # If JD provided, use JD skills; otherwise use role's default skills
                if jd_content:
                    target_skills = self._extract_skills_from_jd(jd_content)
                else:
                    target_skills = self.role_skills.get(final_target_role, [])
            else:
                # Auto-detect (empty target_role)
                final_target_role = self._predict_role_from_text(text)
                if jd_content:
                    target_skills = self._extract_skills_from_jd(jd_content)
                else:
            ...
            # --- HR REQUIREMENT CHECKS (ENHANCED VERSION) ---
